...
def process_output(output_details, output) -> 'list[float]':
    ...
    # e.g. yolov7 doesn't have the batch dim, which is weird but OK
    # just don't normalize then
...
def object_detection_nms(raw_scores: list, width_height: int, threshold: float):
    if len(raw_scores) == 0:
        return raw_scores
    ...
    for label in np.unique(d_labels):
        ...
        fixed_boxes = []
        for bb in boxes:
            ymin, xmin, ymax, xmax = bb * width_height
            fixed_boxes.append(np.array([ xmin, ymin, xmax, ymax ]))
