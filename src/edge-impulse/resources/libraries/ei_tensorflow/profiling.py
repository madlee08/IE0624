...
def make_predictions(mode, model, validation_dataset, Y_test, train_dataset,
                            Y_train, test_dataset, Y_real_test, akida_model_path, num_classes,
    ...
    return make_predictions_tflite(mode, model, validation_dataset, Y_test,
                                   train_dataset, Y_train, test_dataset, Y_real_test, num_classes, output_directory, item_feature_axes)

def make_predictions_tflite(mode, model, validation_dataset, Y_test, train_dataset, Y_train, test_dataset, Y_real_test, num_classes, output_directory, item_feature_axes: list = None):
    ...
    if mode == 'object-detection':
        ...
        prediction = tflite_predict_yolov2(model, validation_dataset, Y_test, len(Y_test), num_classes, output_directory)
        ...
        prediction = tflite_predict(model, validation_dataset, len(Y_test), item_feature_axes)
        if (not train_dataset is None) and (not Y_train is None):
            prediction_train = tflite_predict(model, train_dataset, len(Y_train), item_feature_axes)
        if (not test_dataset is None) and (not Y_real_test is None):
            prediction_test = tflite_predict(model, test_dataset, len(Y_real_test), item_feature_axes)
...
def profile_model(model_type, model, model_file, validation_dataset, Y_test, X_samples, Y_samples,
    ...
    if mode != 'visual-anomaly':
        prediction, prediction_train, prediction_test = make_predictions(mode, model, validation_dataset, Y_test,
                                                                         ...
                                                                         Y_real_test, akida_model_path, num_classes,
