...
def _check_shapes(y_true, y_pred):
    if y_true.shape != y_pred.shape:
        raise Exception("shapes of y_true & y_pred must match"
                        f" but were {y_true.shape} & {y_pred.shape}")
    if len(y_true.shape) != 3:
        raise Exception("expected y_ data to be 3d => (B, H, W)"
                        f" but y_ data was {y_true.shape}")
