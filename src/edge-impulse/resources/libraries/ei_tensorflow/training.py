...
# Feeds values from our memory mapped training data into the tensorflow dataset
def create_generator_object_detection(X_values: np.memmap, width_height: int, num_channels: int,
                                      Y_values: list, num_classes: int, augment: bool):
    data_length = len(X_values)
    if augment:
        augmenter = Augmentation(width_height, num_channels)
    def gen():
        for ix in range(data_length):
            ...
            boxes, classes = ei_tensorflow.utils.process_bounding_boxes(
                raw_boxes, width_height, num_classes)
            yield x, (boxes, classes)
    return gen

def get_dataset_object_detection(X_values: np.memmap, width_height: int, num_channels: int, Y_values: list,
                                 num_classes: int, augment: bool):
    # Using the 'args' param of 'from_generator' results in a memory leak, so we instead use a function that
    # returns a generator that wraps the data arrays.
    return tf.data.Dataset.from_generator(
        create_generator_object_detection(X_values, width_height, num_channels, Y_values, num_classes, augment),
        output_signature=(
...
def get_datasets(X_train, Y_train, X_test, Y_test, has_samples, X_samples, Y_samples,
    ...
    if mode == 'object-detection':
        ...
        width, height, num_channels = reshape_to
        if width != height:
            raise Exception(f"Only square inputs are supported; not {reshape_to}")
        width_height = width
        if num_channels not in [1, 3]:
            raise Exception(f"Only single channel, or RGB images are supported")

        train_dataset = get_dataset_object_detection(X_train, width_height, num_channels,
             Y_train, classes, augment=augmentation_enabled)
        validation_dataset = get_dataset_object_detection(X_test, width_height, num_channels,
